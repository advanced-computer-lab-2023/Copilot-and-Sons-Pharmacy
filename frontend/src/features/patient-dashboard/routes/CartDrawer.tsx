...
const Cart: React.FC<CartProps> = ({ isOpen, onClose }) => {
  const { cart, removeFromCartProvider,totalPrice, incrementQuantityProvider,updateQuantityProvider, decrementQuantityProvider,addToCartProvider,clearCartProvider } = useCart();
 ...
 async function viewCart() {
  ...
  return (
    <Drawer anchor="right" open={isOpen} onClose={onClose} style={{height:'100%'}}>
  ...
  <CardContent>
    <img src={item.medicine.Image} alt={item.medicine.name} style={{ width: 100, height: 100 }} />
    <Typography variant="h5" style={{ fontSize: 16 }}>{item.medicine.name}</Typography>
    <Typography variant="body2" style={{ fontSize: 14 }}>${item.medicine.price *item.quantity}</Typography>
    <Typography variant="body2" style={{ fontSize: 12 }}>Quantity: {item.quantity}</Typography>
  </CardContent>
  <CardActions>
    <Button
      size="small"
    ...
    <Button
      size="small"
      variant="outlined"
      onClick={() => incrementQuantity(item.medicine, item.quantity)}
      style={{ fontSize: 12 }}
    >
      +
    </Button>
    <TextField
                    type="text"
                    label="Qty"
                    onBlur={(e) => updateQuantity(item.medicine, parseInt(e.target.value, 10))}
                    size="small"
                    variant="outlined"
                    style={{marginLeft:10 }}
                    InputProps={{
    ...
    <Button
      size="small"
      variant="outlined"
      onClick={() => decrementQuantity(item.medicine._id, item.quantity)}
      style={{ fontSize: 12 }}
    >
      -
    </Button>
      ...
      <Button variant="contained" color="primary"   fullWidth >
        Checkout <ArrowRightAltOutlined style={{marginLeft:20,fontSize:40  }}/>
      </Button>
    </div>
  </Drawer>
  );
